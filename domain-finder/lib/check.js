...
// Used by both server.js (local) and netlify/functions/check.mjs (Netlify).
//
...
//            .us, .de, ...) the tool asks the classic WHOIS server instead.
...
const USER_AGENT = "DomainFinder/1.0 (local tool)";

// Settings. server.js and the Netlify function can change these with configure().
const settings = {
  timeoutMs: 12000,           // how long to wait for one registry/WHOIS answer
  maxParallel: 4,             // at most this many RDAP/WHOIS requests in flight at once
  cacheTtlMs: 10 * 60 * 1000, // remember answers for 10 minutes
};
...
// ---------- RDAP bootstrap (which registry server handles which ending) ----------

let bootstrapCache = null; // Map of tld -> base URL
...
// Find the longest suffix of the domain that IANA knows about ("co.uk" before "uk").
async function rdapBaseFor(domain) {
  const map = await loadBootstrap();
  if (!map) return { base: null, known: null }; // bootstrap unavailable: we simply do not know
...
// Returns "registered" | "no-records" | "unknown"
async function dnsCheck(domain) {
  try {
    ...
    return "unknown"; // timeout, SERVFAIL, no internet, ...
  }
}

// Returns { verdict: "taken" | "available" | "unknown", source, detail }
async function rdapCheck(domain) {
  const { base, known } = await rdapBaseFor(domain);

  // The registry for this ending does not publish RDAP at all.
  if (known === false) return { verdict: "unknown", source: "no-rdap", detail: "This domain ending has no RDAP registry." };
  ...
  for (const url of urls) {
    ...
    try {
      ...
      last = { status: 0, error: err && err.name === "AbortError" ? "timeout" : "network" };
    } finally {
      release();
    }
    if (last.status === 200 && last.data) return { verdict: "taken", source: "rdap", detail: registrarName(last.data.entities) };
    if (last.status === 404) {
      // rdap.org answers 404 both for "no such domain" AND for "I have no server
      // for this ending". Only the first one means the name is free.
      const title = last.data && typeof last.data.title === "string" ? last.data.title : "";
      if (/no rdap service/i.test(title)) return { verdict: "unknown", source: "no-rdap", detail: "This domain ending has no RDAP registry." };
      return { verdict: "available", source: "rdap", detail: null };
    }
    // 429 (rate limited), 5xx, or a network problem: try the next URL, if any.
  }

  if (last.status === 429) return { verdict: "unknown", source: "rdap", detail: "The registry is rate-limiting lookups. Wait a minute and check this one again." };
  if (last.status) return { verdict: "unknown", source: "rdap", detail: "The registry returned an unexpected response (" + last.status + ")." };
...
// ---------- WHOIS (the older lookup system, used when RDAP is not offered) ----------

function whoisQuery(server, query) {
  return new Promise((resolve, reject) => {
    const socket = net.createConnection({ host: server, port: 43 });
    let buf = "";
    const timer = setTimeout(() => { socket.destroy(); reject(new Error("timeout")); }, settings.timeoutMs);
...
// IANA's WHOIS tells us which server handles each ending ("io" -> whois.nic.io).
const whoisServerCache = new Map(); // tld -> server name, or null if IANA lists none
async function whoisServerFor(tld) {
  ...
  try {
    ...
    return null; // do not cache a network failure
  }
  whoisServerCache.set(tld, server);
  return server;
}

// Phrases registries use to say "nobody owns this name".
const WHOIS_FREE = /^[\s%#>*]*(domain not found|no data found|no match|not found|no entries found|nothing found|no such domain|no object found|object does not exist|status:\s*(free|available)|.*(is available for registration|not registered|is free|queried object does not exist|domain not found))/im;
// Phrases that only appear when a record exists.
const WHOIS_TAKEN = /^[\s%#>*]*(domain name|domain|registrar|registry domain id|creation date|created|registered on|nserver|name server)\s*:/im;
...
async function whoisCheck(domain) {
  ...
  try {
    ...
    return { verdict: "unknown", source: "whois", detail: err.message === "timeout" ? "The WHOIS server took too long to respond." : "Could not reach the WHOIS server." };
  } finally {
    release();
  }
  ...
  if (WHOIS_LIMIT.test(text)) return { verdict: "unknown", source: "whois", detail: "The WHOIS server is rate-limiting lookups. Wait a minute and check this one again." };
...
async function checkDomain(domain) {
  ...
  if (dnsResult === "registered") {
    // Name servers exist, so someone owns it. No need to ask the registry.
    result = { status: "taken", verified: true, source: "dns", note: "This domain has live name servers, so it is already registered." };
  } else {
    // Ask the registry. RDAP first; if that cannot answer, classic WHOIS.
    ...
    if (r.verdict === "available") {
      ...
      result = { status: "available", verified: false, source: "dns", note: "Probably available: no name servers found, but the registry could not confirm it. " + (r.detail || "") + " Confirm at the registrar before you rely on it." };
    } else {
...
// One function both servers call. Returns { code, body } ready to send as JSON.
async function handleCheckRequest(rawDomain) {
  const domain = cleanDomain(rawDomain);
  if (!domain) return { code: 400, body: { ok: false, error: "Please enter a domain name." } };
  if (!looksLikeDomain(domain)) return { code: 400, body: { ok: false, domain, error: `"${domain}" does not look like a valid domain name.` } };
  try {
    const result = await checkDomain(domain);
    return { code: 200, body: { ok: true, domain, ...result } };
  } catch (err) {
    return { code: 502, body: { ok: false, domain, error: "Check failed: " + (err && err.message ? err.message : "unknown error") } };
  }
}

module.exports = { configure, cleanDomain, looksLikeDomain, loadBootstrap, checkDomain, handleCheckRequest };
