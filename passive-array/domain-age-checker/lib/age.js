// Domain Age Checker - the lookup logic.
// Used by server.js (local) and by netlify/functions/domain-age.mjs (Netlify).
// It asks the public RDAP registry for the domain's registration date.

const USER_AGENT = "DomainAgeChecker/1.0 (local tool)";

const settings = { timeoutMs: 12000 };
...
async function handleAgeRequest(raw) {
  const domain = cleanDomain(raw);

  if (!domain) return { code: 400, body: { ok: false, error: "Please enter a domain name." } };
  if (!looksLikeDomain(domain)) return { code: 400, body: { ok: false, error: `"${domain}" does not look like a valid domain name.` } };

  try {
    const found = await findDomainRecord(domain);
    if (!found.data) {
      if (found.status === 404) return { code: 404, body: { ok: false, domain, error: "No registration record found. The domain may be unregistered, or its registry does not publish RDAP data." } };
      if (found.status === 429) return { code: 429, body: { ok: false, domain, error: "The registry is rate-limiting lookups. Please wait a minute and try again." } };
      return { code: 502, body: { ok: false, domain, error: `The registry returned an unexpected response (${found.status}).` } };
    }
    ...
    return { code: 502, body: { ok: false, domain, error: msg } };
  }
}

module.exports = { configure, cleanDomain, looksLikeDomain, handleAgeRequest };
